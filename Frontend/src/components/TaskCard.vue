<template>
  <div class="card text-white bg-primary m-3 float-left" style="width: 18rem; height: 14rem;">
    <div class="card-header">
      <span
        v-on:keydown.enter="updateCard()" v-on:input="card.header = $event.target.innerText"
    ...
    <div class="card-body">
      <h5 v-on:keydown.enter="updateCard()" v-on:input="card.title = $event.target.innerText"
        v-bind:contenteditable="!disable" class="card-title">{{card.title}}</h5>
      <p v-on:keydown.enter="updateCard()" v-on:input="card.text = $event.target.innerText"
